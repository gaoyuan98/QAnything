...
class DMCompiler(compiler.SQLCompiler):
    ...
    def visit_update(self, update_stmt, **kw):
        ...
        if hasattr(self.statement, 'table') and update_stmt.table.name != self.statement.table.name:
                ...
                if need_get_primary_flag:
                    ...
                    if len(result) != 1:
                        ...
                        self.inserted_primary_key = [
                            self.compiled._inserted_primary_key_from_lastrowid_getter(result, temp_params)]
                        ...
                        self.inserted_primary_key = [
                            self.compiled._inserted_primary_key_from_lastrowid_getter(result[0], temp_params)]
                        return self.inserted_primary_key
...
class DMDialect(default.DefaultDialect):
    ...
    def get_table_options(self, connection, table_name, schema=None, **kw):
        ...
        if row:
            get_table_flag = True
            if row[1] == 'ENABLED':
                options['dm_compress'] = 'BASIC'
            if row[3] is not None:
                options['dm_tablespace'] = row[3]
    ...
    def get_columns(self, connection, table_name, schema=None, **kw):
        ...
        text = "SELECT a_tab_cols.column_name, "\
                "\nCASE LOWER(a_tab_cols.data_type)"\
                "\n WHEN 'blob' THEN"\
                "\n     CASE syscol.scale"\
    ...
    def get_multi_columns(self, connection, schema, filter_names, scope, kind, dblink=None, **kw):
        ...
        i8_vec_pattern = r'VECTOR\((\d+),INT8\)'
        f32_vec_pattern = r'VECTOR\((\d+),FLOAT32\)'
        f64_vec_pattern = r'VECTOR\((\d+),FLOAT64\)'

        spec_list = []
        for row_dict in spec_result:
            if row_dict['data_type'] in ('JSON', 'JSONB') or re.search(i8_vec_pattern, row_dict['data_type']) or re.search(f32_vec_pattern, row_dict['data_type']) or re.search(f64_vec_pattern, row_dict['data_type']):
    ...
    def get_table_comment(
        ...
        **kw
        ...
        row_dict = result.first()
        if row_dict is not None:
            comment_str = row_dict[0]
            get_table_flag = True
    ...
    def get_multi_check_constraints(self, connection, *, schema, filter_names, dblink=None, scope, kind, include_all=False, **kw):
        ...
        query = "SELECT constraint_name AS constraint_name, table_name AS table_name, "\
        ...
        for row_dict in run_result:
            constraint_name = row_dict['constraint_name']
            table_name = row_dict['table_name']
            search_condition = row_dict['search_condition']
            table_name = self.normalize_name(table_name)
            constraint_name = self.normalize_name(constraint_name)

            if constraint_name is not None and (
                    include_all or not not_null.match(search_condition)
            ):
                new_check_cons = {}
                new_check_cons['name'] = constraint_name
                new_check_cons['sqltext'] = search_condition
                check_constraints[(schema, table_name)].append(new_check_cons)
    ...
    def get_check_constraints(
        ...
        result = []
        for row_dict in constraint_data:
            (constraint_name, table_name, search_condition) = row_dict[0:3]
            table_name = self.normalize_name(table_name)
            constraint_name = self.normalize_name(constraint_name)
