...
class VectorAdaptor:
    ...
    def client_create_vector_ivf_index(self, index):
        if index is None:
            raise ValueError("index is None")
        column = index.column
        skip_existing = index.skip_existing
        metric_name = index.metric_name
        index_name = index.index_name
        percentage_value = index.percentage_value
        num_of_partitions = index.num_of_partitions
        self.create_vector_ivf_index(column, skip_existing, metric_name, index_name, percentage_value, num_of_partitions)
    ...
    def create_vector_ivf_index(
        ...
        table_name = conn.dialect.identifier_preparer.quote(table_name)
        column_name = conn.dialect.identifier_preparer.quote(column_name)
    ...
    def client_create_vector_hnsw_index(self, index):
        if index is None:
            raise ValueError("index is None")
        column = index.column
        skip_existing = index.skip_existing
        ...
        index_name = index.index_name
        percentage_value = index.percentage_value
        max_connection = index.max_connection
        ef_construction = index.ef_construction
        self.create_vector_hnsw_index(column, skip_existing, metric_name, index_name, percentage_value, max_connection, ef_construction)
    ...
    def _check_index_match(self, conn, schema_name, table_name, column_name, index_name):
        query_str = ("SELECT index_name FROM ALL_IND_COLUMNS WHERE TABLE_OWNER = :owner AND TABLE_NAME = :table_name "
        ...
        try:
            for row_dict in columns:
                ...
                if row_dict['name'] == 'embedding':
                    if (type(row_dict['type']) != VECTOR or row_dict['type'].dim != self._vector_dim or
